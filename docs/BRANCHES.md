...
| Review the **small-store growth foundation** (desktop SQLite, secure sync, rollback) | `git fetch origin && git checkout codex/small-store-growth-foundation` |
...
main
 └── feat/pos-module          (POS / earlier integration — pushed earlier)
       └── release/biashara-phase4   (stable line: U-track + POS + Phase 6 models/skills, …)
             ...
             └── codex/small-store-growth-foundation   (desktop persistence, idempotent sync, security docs)
...
| Date | Change |
|------|--------|
| 2026-09-04 | Added `codex/small-store-growth-foundation` as the review and rollback branch for desktop SQLite persistence, versioned phone sync, and stock reconciliation hardening. |
