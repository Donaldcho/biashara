...
/**
 * Validates **v3 → v7** migrations preserve existing product and transaction rows
...
class AppDatabaseMigrationTest {
    ...
    @Test
    fun migrate3To7_preservesProductsAndTransactionsAndSeedsSettings() {
        ...
        db.query(
            "SELECT amount, description, payment_method, tax_rate, tax_amount FROM transactions WHERE id = 1",
        ).use { c ->
            assertTrue(c.moveToFirst())
